...
var boltzmann = module.exports = pattern.extend({
	...
	setHidden: function(index, input) {
		var i, j, k, result = [];
	...
	calculateEnergy: function(l1, l2) {
		var j, k, energy = 0.0;
		...
		for(j = 0; j < this.neurons[0].length; j++) {
			...
			// For every weight, ...
			for(k = 0; k < this.weights[0][j].length; k++) {
				energy -= this.neurons[0][j].state * this.neurons[1][k].state * this.weights[0][j][k];
			}
		}
