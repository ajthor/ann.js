...
var boltzmann = module.exports = pattern.extend({
	...
	run: function(input) {
		return this.runVisible(input);
		return this.runHidden();
	},

	runVisible: function(input) {
		var i, j, k, result = [];
		// Copy the input array to avoid changes to the original.
		// Set it to be the 'visible' layer.
		result[0] = input ? input.slice() : [];
		...
		for(j = 0; j < this.neurons[0].length; j++) {
			if(typeof result[0][j] !== 'undefined') {
				this.neurons[0][j].state = result[0][j];
			}
	...
	runHidden: function(index, input) {
		var i, j, k, result = [];

		if((typeof input === 'undefined') || !(result[index])) {
			input = index || [];
			index = this.neurons.length-1;
		}
		// Copy the input array to avoid changes to the original.
		// Set it to be the 'hidden' layer.
		result[index] = input.slice();

		// Get the state of the last layer.
		for(j = 0; j < this.neurons[index].length; j++) {
			if(typeof result[index][j] !== 'undefined') {
				this.neurons[index][j].state = result[index][j];
				result[index][j] = this.neurons[index][j].state;
			}
			else {
				result[index][j] = this.neurons[index][j].state;
			}
	...
	calculateEnergy: function() {
		var j, k, energy = 0.0;
		// E(v,h) = -1 * (sig(v_i * h_j * w_ij))

		// For each neuron in the visible layer, ...
		for(j = 0; j < this.neurons[0].length; j++) {

			// For every weight, ...
			for(k = 0; k < this.weights[1][j].length; k++) {
				energy -= this.neurons[0][j].state * this.neurons[1][k].state * this.weights[1][j][k];
			}
		}
