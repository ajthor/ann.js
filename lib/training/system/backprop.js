...
var anneal = module.exports = system.extend({
	...
	initialize: function(network, options) {
		...
		for(var i = this.network.weights.length; i--; ) {
	...
	train: function(inputs, ideals) {
		...
		var iterations = this.options.iterations;
		...
		for(i = 0; ((error > threshold) && (i < iterations)); i++) {
			...
			// 'previous' matrices.
			for(j = this.network.weights.length; j--; ) {
	...
	calculateDeltas: function() {
		for(var i = this.deltas.length; i--; ) {
	...
	calculateGradients: function(inputs, ideals) {
		...
		var deltas = [];
		var last = this.network.outputs.length-1;

		var outputs = this.network.outputs;
		var weights = this.network.weights;

		// The output neuron's error signals are just the ideal 
		// output minus what we actually got.
		this.network.outputs[last].forEach(function(value, row, col) {
			deltas[last] = [];
			deltas[last][row] = ideals[row] - value;
		});
		...
		for(i = weights.length-1; i >= 0; i--) {
			...
			// e_current = Array.apply(null, new Array(weights[i].rows)).map(Number.prototype.valueOf,0);;
			
			// console.log(e_last);

			// console.log("out", outputs[i-1].value, "w", weights[i].T.value);

			// deltas[i] = Matrix.dot(weights[i].T, outputs[i-1]);
