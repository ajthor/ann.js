...
var network = module.exports = function(neurons, options) {
	...
	this.options = _.defaults((options || {}), {
		...
		trainingSystem: require("./util/training/training.js").backpropagation
	});
...
_.extend(network.prototype, {
	initialize: function(neurons, options) {
		try {
			...
			for(var i = 0, len = neurons.length; i < len; i++) {
				...
				if(i === len-1) options.hasBias = false;
				// Initialize
				this._layers[i] = new layer(neurons[i], options);
			}
		} catch(e) {
	...
	input: function(input) {
		try {
			...
			// Add in bias to the input layer.
			result.unshift(1);
